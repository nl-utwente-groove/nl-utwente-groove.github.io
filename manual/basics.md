...
During exploration, GROOVE builds up a *transition system*, in which every reached graph is a state and every rule application a transition. By default, transitions are labelled by the names of the applied rules — but see the `transitionLabel` rule property above. The precise formatting of transition labels is further controlled by the system properties `transition brackets` and `transition parameters`. Exploration and the analysis of the resulting transition system are the subject of the [Exploration and verification](manual_verification.html) chapter.
